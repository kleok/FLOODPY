...
def Run_Preprocessing(gpt_exe,
    ...
    Flood_image=os.path.join(S1_GRD_orbit_dir, flood_S1_image)
    assert (os.path.exists(Flood_image))
    
    
    S1_images_df = pd.read_pickle(baseline_images)   
    ...
    print(Master_image)
    Master_image_filename=os.path.join(S1_GRD_orbit_dir,Master_image)
    ...
    for pair in One_master_combinations:
        Slave_image=pair[1]
    ...
    Slave_datetimes_refined = check_coregistration_validity(Master_datetime, Slave_datetimes, Preprocessing_dir, 0.5)
    hdf5_stack_file=Create_dhf5_stack_file(Master_datetime, Slave_datetimes_refined, Preprocessing_dir)
