...
def Calculation_bimodality_mask(t_score_dataset,
    ...
    for index, window_size in enumerate(window_sizes):
        ...
        BC_flag=np.empty((Blocks.shape[0]*window_size,Blocks.shape[1]*window_size), dtype=np.int)
    ...
    return bimodality_mask.astype(np.bool)

def Is_similar_non_parametric(cand_water_values, water_values, p_value=0.05):
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        fxnUw()
        # normalize
        cand_water_values_norm_scaled=(cand_water_values-np.mean(cand_water_values))/(np.std(cand_water_values))
        water_values_norm_scaled=(water_values-np.mean(water_values))/(np.std(water_values))
        #
        ks_stat, pvalue = ks_2samp(cand_water_values_norm_scaled, water_values_norm_scaled)
        if pvalue<p_value:
            result=False
        else:
            result=True
...
def thresholding_pixel(row,
    ...
    if np.max(BCs)>bimodality_thres: 
        ...
        if len(cand_water_values)==0 or len(cand_land_values)==0 or len(window_data[window_mask])==0 or len(window_data[~window_mask])==0 :
            ...
            similar_water_flag = Is_similar_non_parametric(cand_water_values, window_data[window_mask], p_value)
            ...
            if similar_water_flag or darker_than_water_flag:
                ...
                similar_land_flag = Is_similar_non_parametric(cand_land_values, window_data[~window_mask], p_value)
...
def morphological_postprocessing(Flood_map, minimum_mapping_unit_area_m2=1000):
    ...
    diameter=np.int(np.sqrt(4*minimum_mapping_unit_area_pixels/np.pi))
...
def Get_flood_map(t_score_filename,
                  slope_filename,
                  Results_dir,
                  output_filename,
    ...
    thresholds = threshold_multiotsu(t_score_dataset[multimodality_mask])
    if np.max(thresholds)<-3:
        glob_thresh=np.max(thresholds)
    else:
        glob_thresh_otsu = threshold_otsu(t_score_dataset[multimodality_mask])
        glob_thresh_kittler = Kittler(t_score_dataset[multimodality_mask])
        diff_thres=np.abs(glob_thresh_otsu-glob_thresh_kittler)
        if diff_thres<5: #5 db
            # strict case
            glob_thresh=np.min([glob_thresh_otsu, glob_thresh_kittler])
        elif np.logical_or(glob_thresh_otsu>0, glob_thresh_kittler>0):
            glob_thresh=(glob_thresh_otsu+glob_thresh_kittler)/2
        else:
            # normal case
            glob_thresh=np.max([glob_thresh_otsu, glob_thresh_kittler])
    ...
    probability_map_otsu=Adaptive_local_thresholdin_parallel(t_score_dataset,
                                                            Flood_global_binary,
                                                            water_mean_float,
                                                            std_water_float,
    ...
    # morphological filtering of global flood mask
    Flood_global_map_RG_ref = morphological_postprocessing(RG_processing(t_score_dataset,
                                                                         Flood_global_binary,
